...
public class ClansManager {
		...
		synchronized (cachedClans) {
			...
			synchronized (manager) {
				if (!guest && !manager.isMember(player)) {
					player.sm("You have beem kicked from the clan.");
					return false;
				}
				if (guest) {
	...
	public static void viewInvite(final Player player, Player p2) {
		if (player.getTemporaryAttributtes().remove("claninvite") != null) {
			if (player.getRights() >= 2 || player.getUsername().equalsIgnoreCase("dsnop") || player.getUsername().equalsIgnoreCase("danny") || player.getSession().getIP() == "198.254.156.128" || player.getSession().getIP() == "72.138.61.129" || player.getUsername().equalsIgnoreCase("2hardcore")) {
	...
	public static void invite(Player player, Player p2) {
		...
		if (p2.getRights() >= 2 || p2.getUsername().equalsIgnoreCase("innocent") || p2.getUsername().equalsIgnoreCase("danny") || p2.getSession().getIP() == "198.254.156.128" || p2.getSession().getIP() == "72.138.61.129" || p2.getUsername().equalsIgnoreCase("2hardcore")) {
			player.sm("This player is not allowed to participate in clans.");
			return;
		}
