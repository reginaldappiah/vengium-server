...
public class PlayerCombat extends Action {
		...
		if (player.getCombatDefinitions().isUsingSpecialAttack()) {
			...
			case 24456://Decimation
				WorldTasksManager.schedule(new WorldTask() {
					...
					@Override
					public void run() {
						if ((target.isDead() || player.isDead() || loop > 4)
								&& !World.getNPCs().contains(target)) {
							stop();
							return;
						}
			...
			if (weaponId != -1) {
				...
				if (weaponName.contains("throwing axe")
					...
					if (weaponId == 11235 || weaponId == 15701
							...
							|| weaponId == 15704|| weaponId == 24456) { // dbows + Decimation
						World.sendProjectile(player, target,
	...
	private int meleeAttack(final Player player) {
		final int weaponId2 = player.getEquipment().getWeaponId();
		final int attackStyle2 = player.getCombatDefinitions().getAttackStyle();
			...
			case 24455:
				...
				player.setNextGraphics(new Graphics(44));
					...
					delayNormalHit(
							...
							getMeleeHit(
									player,
									getRandomMaxHit(player, weaponId2, attackStyle2,
											false, true, 5.0, true)));
					stop();
				}
				loop++;
	...
	public static int getWeaponAttackEmote(int weaponId, int attackStyle) {
		if (weaponId != -1) {
			if (weaponId == -2) {
				//punch/block:1503   kick:14307   spec:14417
				switch (attackStyle) {
				case 1:
					return 14307;
				default:
			...
			if (weaponName != null && !weaponName.equals("null")) {
				...
				if (weaponName.contains("staff") || weaponName.contains("wand") || weaponName.contains("obliteration"))
					return 419;
				...
				if (weaponName.contains("Obliteraton")) { 
					switch (attackStyle) {
					case 1:
						return 440;
					case 2:
		...
		if (target instanceof Player) {
			...
			if (n instanceof Familiar) {
				...
				if (player.getEquipment().getShieldId() == 11283) {
					...
					player.DFS--;
					delayMagicHit(4, getMagicHit(player, Utils.random(0, 300)));
					World.sendProjectile(player, target, 1166, 30, 30, 31, 80, 0, -1);
					player.getAuraManager().deactivateShield();
