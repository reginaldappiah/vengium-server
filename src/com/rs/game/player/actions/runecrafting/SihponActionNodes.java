...
public class SihponActionNodes extends Action {
	...
	public boolean checkAll(final Player player) {
		...
		if (!started && !player.withinDistance(node, 6))
			return true;
		if (!World.containsObjectWithId(node, node.getId())) {// 9
																// errors:!@#!@#!
			stop(player);
			return false;
		}
	...
	private void processNodeDestroy(final Player player) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				World.spawnObject(new WorldObject(node.getId(), node.getType(), node.getRotation(), node.getX(), node.getY(), node.getPlane(), node.getLife()));
				stop();
			}
		}, 2);
	}
