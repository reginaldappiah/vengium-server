...
import com.rs.game.player.dialogues.impl.POHPortal;
...
public class House implements Serializable {
	...
	private transient boolean loaded;//the type was here 
	...
	public void createRoom(int slot) {
		System.out.println("Create room with slot: " + slot);// try relaunch
																// savfev before
																// that
		Room[] rooms = HouseConstants.Room.values();
		if (slot >= rooms.length) {
			System.out.println("Slot is more than expected");
			return;
		}
		...
		System.out.println("Opening the dialogue");
	...
	public void createRoom(RoomReference room) {
		if (player.getInventory().getCoinsAmount() < room.room.getPrice()) { // build
																				// work?
			player.getPackets().sendGameMessage("You don't have enough coins to build this room.");
			return;
		}
	...
	// Used for inter 396 everything in objects stooed working and i always
	// spawn to that werid place wtflo yh
	...
	public void setPlayer(Player player) {
		this.player = player;//here we go this is itw
		if(location == null)
			location = POHLocation.HOME;
	}
