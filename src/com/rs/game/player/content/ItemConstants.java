...
public class ItemConstants {
	...
	public static boolean canWear(Item item, Player player) {
		...
		if ((item.getId() == 20769 || item.getId() == 20771)) {
			...
			if (!player.isCompletedFightCaves()) {
				player.getPackets().sendGameMessage("You must complete the fightcaves to wear this.");
				return false;
			}
		}
		 ...
		 * (player.getDominionTower().getKilledBossesCount() < 50) { player.sm(
	...
	public static boolean isTradeable(Item item) {
		...
		if (item.getDefinitions().getName().toLowerCase().contains("flaming skull"))
			return false;
		switch (item.getId()) {
		case 29980: // RSMV Token
		case 29992: // Rainbow Boots
		case 29993: // Rainbow Goggles
		case 29994: // Dark Shadow Boots
		case 29995: // Light Sahadow Boots
		case 29996: // White Cavalier
		case 29997: // Dark Shadow Cavalier
		case 6570: //firecape
		case 6529: //tokkul
