...
public final class PlayerLook {
	...
	public static void openCharacterCustomizing(Player player) {
		...
		player.getPackets()
				.sendUnlockIComponentOptionSlots(1028, 128, 0, 50, 0);
		player.getPackets().sendUnlockIComponentOptionSlots(1028, 132, 0, 250,
				0);
		player.getPackets().sendConfigByFile(8093,
	...
	public static void handleCharacterCustomizingButtons(Player player,
		...
		if (buttonId == 138) { // confirm
			...
			player.lock(10);
			player.getDialogueManager().startDialogue("CutsceneToggle");
	...
	public static void setGender(Player player, boolean male) {
		...
		setDesign(player, index1 != null ? index1 : 0, index2 != null ? index2
				: 0);
		player.getAppearence().generateAppearenceData();
		player.getPackets().sendConfigByFile(8093, male ? 0 : 1);
	}
