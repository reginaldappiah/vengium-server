...
public class NPC extends Entity implements Serializable {
						...
						if (getFreezeDelay() < Utils.currentTimeMillis()) {
							if (((hasRandomWalk())) // temporary
								...
								if (getMapAreaNameHash() != -1) {
									if (!MapAreas.isAtArea(getMapAreaNameHash(), this)) {//that noob pissed me off he only put 1 class and then he want get paid? wtf he didnt do anything
	...
	public void handlePointsNPCs() {
		...
		case 7741:
			...
			killer.setNpcPoints(killer.getdungpoints() + 750); // amount of
																// points given
			killer.getPackets().sendGameMessage("<col=ff0000>You currently have : " + killer.getdungpoints() + " Dungeoneering Points.");
			break;
		}
	}
	...
	public void sendDrop(Player player, Drop drop) {
		...
		if (random >= 95 && random <= 100) {
			if (this.getId() == 116 || this.getId() == 4292 || this.getId() == 4291 || this.getId() == 6078 || this.getId() == 6079 || this.getId() == 6080 && drop.getItemId() == 8844) {
				WGuildControler.dropDefender(player, this);
				return;
			}
		}
		if (dropName.contains("pernix") || dropName.contains("torva") || dropName.contains("virtus") || dropName.contains("bandos") || dropName.contains("armadyl") || dropName.contains("sara") || dropName.contains("zamorak") || dropName.contains("arcane") || dropName.contains("divine") || dropName.contains("spectral") || dropName.contains("elysian") || dropName.contains("dragonfire shield") || dropName.contains("whip") || dropName.contains("dark") || dropName.contains("stead") || dropName.contains("glaiven") || dropName.contains("ragefire")) {
			World.sendWorldMessage("<col=00ff00>" + Utils.formatPlayerNameForDisplay(player.getUsername()) + " has received " + dropName + " as drop</col>", false);
		}
	}
