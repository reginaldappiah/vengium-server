...
public final class CastleWars {
	...
	public static void setGloves(Player player, Item hands) {
		player.getEquipment().getItems().set(Equipment.SLOT_HANDS, hands);
		player.getEquipment().refresh(Equipment.SLOT_HANDS);
		player.getAppearence().generateAppearenceData();
	}

	public static void setAmulet(Player player, Item amulet) {
		player.getEquipment().getItems().set(Equipment.SLOT_AMULET, amulet);
	...
	private static class PlayingGame extends TimerTask {
		...
		public void takeFlag(Player player, int team, int flagTeam, WorldObject object, boolean droped) {
			...
			if (!droped) {
				...
				World.removeObject(object, true);//test it i hopefully it wokr
				if (flagTeam == team) {
					makeSafe(flagTeam);
					return;
				}
