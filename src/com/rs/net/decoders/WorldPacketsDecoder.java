...
public final class WorldPacketsDecoder extends Decoder {
    ...
    // private final static int PLAYER_OPTION_7_PACKET = 77;
    ...
    private final static int REPORT_ABUSE_PACKET = 16;
    ...
    public static void decodeLogicPacket(final Player player, LogicPacket packet) {
        ...
        if (packetId == WALKING_PACKET || packetId == MINI_WALKING_PACKET) {
            ...
            for (int step = 0; step < steps; step++)
                if (!player.addWalkSteps(baseX + stream.readUnsignedByte(), baseY + stream.readUnsignedByte(), 25,
                                         true)) {
                    break;
                }
        }
            ...
            final WorldObject object = !player.isAtDynamicRegion() ? mapObject : new WorldObject(objectId, mapObject
                    .getType(), mapObject.getRotation(), x, y, player.getPlane());
            ...
            if (other == null || other.isDead() || other.hasFinished() || !player.getMapRegionsIds().contains(other
                                                                                                                      .getRegionId())) {
                return;
            }
            if (player.getLockDelay() > Utils.currentTimeMillis()) {
            ...
            if (player.getRights() == 3) {
                player.getDialogueManager().startDialogue("ModCat", playerIndex);
            }
            if (player.getRights() == 4) {
                player.getDialogueManager().startDialogue("AdminCat", playerIndex);
            }
        }
            ...
            int playerIndex = stream.readUnsignedShortLE128();
            Player p2 = World.getPlayers().get(playerIndex);
            if (p2 == null || p2 == player || p2.isDead() || p2.hasFinished() || !player.getMapRegionsIds().contains
                    (p2.getRegionId())) {
                return;
            }
            if (player.isLocked()) {
            ...
            switch (interfaceId) {
                ...
                case 747:
                    ...
                    if ((interfaceId == 747 && componentId == 15) || (interfaceId == 662 && componentId == 65) ||
                        ...
                        if (!player.isCanPvp() || !p2.isCanPvp()) {
                            player.getPackets().sendGameMessage("You can only attack players in a player-vs-player " +
                                                                        "area.");
                            return;
                        }
                        if (!player.getFamiliar().canAttack(p2)) {
                            ...
                            player.getFamiliar().setSpecial(interfaceId == 662 && componentId == 74 || interfaceId ==
                                    747 && componentId == 18);
                    ...
                    switch (componentId) {
                        ...
                        case 23:
                            if (Magic.checkCombatSpell(player, componentId, 1, false)) {
                                ...
                                if (!player.isCanPvp() || !p2.isCanPvp()) {
                                    player.getPackets().sendGameMessage("You can only attack players in a " +
                                ...
                                if (!p2.isAtMultiArea() || !player.isAtMultiArea()) {
                                    if (player.getAttackedBy() != p2 && player.getAttackedByDelay() > Utils
                                            .currentTimeMillis()) {
                                        player.getPackets().sendGameMessage("That " + (player.getAttackedBy()
                                                                                               instanceof Player ? "player" : "npc") + " is already in combat.");
                                        return;
                                    }
            ...
            else if (player.getTemporaryAttributtes().get("answer_trivia") == Boolean.TRUE) {
                TriviaBot.verifyAnswer(player, value);
                player.getTemporaryAttributtes().put("answer_trivia", Boolean.FALSE);
                return;
            }
            ...
            else if (player.getTemporaryAttributtes().remove("triviaanswer") != null) {
                TriviaBot.verifyAnswer(player, value);
                return;
            }
        }
    ...
    public void processPackets(final int packetId, InputStream stream, int length) {
        ...
        if (packetId == PING_PACKET) {
                ...
                switch (spellId) {
                    case 59:// High Alch
                        if (player.getSkills().getLevel(Skills.MAGIC) < 55) {
                            player.getPackets().sendGameMessage("You do not have the required level to cast this " +
                                                                        "spell.");
                            return;
                        }
                        if (itemId == 995) {
                        ...
                        if (player.getEquipment().getWeaponId() == 1401 || player.getEquipment().getWeaponId() ==
                                3054 || player.getEquipment().getWeaponId() == 19323) {
                            if (!player.getInventory().containsItem(561, 1)) {
                                player.getPackets().sendGameMessage("You do not have the required runes to cast this " +
                                                                            "spell.");
                                return;
                            }
                            ...
                            if (!player.getInventory().containsItem(561, 1) || !player.getInventory().containsItem
                                    (554, 5)) {
                                player.getPackets().sendGameMessage("You do not have the required runes to cast this " +
                                                                            "spell.");
                                return;
        ...
        else if (packetId == ACTION_BUTTON1_PACKET || packetId == ACTION_BUTTON2_PACKET || packetId ==
                ACTION_BUTTON4_PACKET || packetId == ACTION_BUTTON5_PACKET || packetId == ACTION_BUTTON6_PACKET ||
                packetId == ACTION_BUTTON7_PACKET || packetId == ACTION_BUTTON8_PACKET || packetId ==
                ACTION_BUTTON3_PACKET || packetId == ACTION_BUTTON9_PACKET || packetId == ACTION_BUTTON10_PACKET) {
            ...
            if (player.getInterfaceManager().containsInterface(1108)) {
                ...
                if (value.length() != 6) {
                    player.getDialogueManager().startDialogue("SimpleMessage", "The HEX yell color you wanted to pick" +
                            " cannot be longer and shorter then 6.");
                }
                else if (Utils.containsInvalidCharacter(value) || value.contains("_")) {
                    player.getDialogueManager().startDialogue("SimpleMessage", "The requested yell color can only " +
                            "contain numeric and regular characters.");
                }
                else {
                    player.setYellColor(value);
                    player.getDialogueManager().startDialogue("SimpleMessage", "Your yell color has been changed to " +
                            "<col=" + player.getYellColor() + ">" + player.getYellColor() + "</col>.");
                }
            ...
            else if (player.getTemporaryAttributtes().remove("setoutfitname4") != null) {
                if (Utils.invalidAccountName(Utils.formatPlayerNameForProtocol(value))) {
                    player.getPackets().sendGameMessage("<img=15><col=ff0000>Invalid outfit name!");
                    return;
                }
                player.setOutfitName4(value);
                player.getPackets().sendGameMessage("<col=04FF10>Created outfit 4 name!");
            }
            else if (player.getTemporaryAttributtes().remove("setoutfitname3") != null) {
                if (Utils.invalidAccountName(Utils.formatPlayerNameForProtocol(value))) {
                    player.getPackets().sendGameMessage("<img=15><col=ff0000>Invalid outfit name!");
                    return;
                }
                player.setOutfitName3(value);
                player.getPackets().sendGameMessage("<col=04FF10>Created outfit 3 name!");
            }
            else if (player.getTemporaryAttributtes().remove("setoutfitname2") != null) {
                if (Utils.invalidAccountName(Utils.formatPlayerNameForProtocol(value))) {
                    player.getPackets().sendGameMessage("<img=15><col=ff0000>Invalid outfit name!");
                    return;
                }
                player.setOutfitName2(value);
                player.getPackets().sendGameMessage("<col=04FF10>Created outfit 2 name!");
            }
            else if (player.getTemporaryAttributtes().remove("setoutfitname1") != null) {
                if (Utils.invalidAccountName(Utils.formatPlayerNameForProtocol(value))) {
                    player.getPackets().sendGameMessage("<img=15><col=ff0000>Invalid outfit name!");
                    return;
                }
                player.setOutfitName1(value);
                player.getPackets().sendGameMessage("<col=04FF10>Created outfit 1 name!");
            }
        }
            ...
            if (player.getTemporaryAttributtes().remove("withdrawingPouch") == Boolean.TRUE) {
            ...
            else if ((player.getInterfaceManager().containsInterface(762) && player.getInterfaceManager()
                    .containsInterface(763)) || player.getInterfaceManager().containsInterface(11)) {
                if (value < 0) {
                    return;
                }
                ...
                // } else if
                // (player.getTemporaryAttributtes().remove("setting_geoffer_amount")
                // != null) {
                // GrandExchangeOffer offer = (GrandExchangeOffer)
                // player.getTemporaryAttributtes().get("grand_exchange_offer");
                // if (offer != null) {
                // .setPrimaryAmount(value);
                // player.getPackets().sendConfig(1110,
                // offer.getPrimaryAmount());
                // }
                // return;
                // } else if
                // (player.getTemporaryAttributtes().remove("setting_geoffer_price")
                // != null) {
                // GrandExchangeOffer offer = (GrandExchangeOffer)
                // player.getTemporaryAttributtes().get("grand_exchange_offer");
                // if (value == 0) {
                ...
                if (player.getEquipment().wearingArmour()) {
                    ...
                    player.getDialogueManager().startDialogue("SimpleMessage", "You cannot do this while having " +
                            "armour on!");
                    return;
                }
            ...
            if (Utils.getInterfaceDefinitionsSize() <= fromInterfaceId || Utils.getInterfaceDefinitionsSize() <=
                    toInterfaceId) {
                return;
            }
            ...
            if (fromComponentId != -1 && Utils.getInterfaceDefinitionsComponentsSize(fromInterfaceId) <=
                    fromComponentId) {
                return;
            }
            ...
            if (fromInterfaceId == Inventory.INVENTORY_INTERFACE && fromComponentId == 0 && toInterfaceId ==
                    Inventory.INVENTORY_INTERFACE && toComponentId == 0) {
                toSlot -= 28;
			 ...
			 */
                ...
                packetId == PLAYER_OPTION_2_PACKET || packetId == PLAYER_OPTION_4_PACKET || packetId ==
                PLAYER_OPTION_1_PACKET || packetId == ATTACK_NPC || packetId == PLAYER_OPTION_5_PACKET || packetId ==
                INTERFACE_ON_PLAYER || packetId == INTERFACE_ON_NPC || packetId == NPC_CLICK1_PACKET || packetId ==
                NPC_CLICK2_PACKET || packetId == NPC_CLICK3_PACKET || packetId == OBJECT_CLICK1_PACKET || packetId ==
                SWITCH_INTERFACE_ITEM_PACKET || packetId == OBJECT_CLICK2_PACKET || packetId == OBJECT_CLICK3_PACKET
                || packetId == OBJECT_CLICK4_PACKET || packetId == OBJECT_CLICK5_PACKET || packetId ==
                INTERFACE_ON_OBJECT) {
            ...
            // GrandExchangeOffer offer = new
            // GrandExchangeOffer(player.getDisplayName(), (int)
            // player.getTemporaryAttributtes().get("grand_exchange_slot_clicked"),
            // item, 1, (int) (item.getDefinitions().getValue()), Type.BUY);
            ...
            // player.getTemporaryAttributtes().put("grand_exchange_offer",
            // offer);
        }
            ...
            if (player.getMuted() > Utils.currentTimeMillis()) {
                player.getPackets().sendGameMessage("You temporary muted.");
                return;
            }
            if (IPMute.isMuted(player.getSession().getIP())) {
                player.getPackets().sendGameMessage("You're account has been permanently IP Muted.");
                return;
            }
            ...
            if (chatType == 1) {
                player.sendPublicChatMessage(new PublicChatMessage(Utils.capitalize(message), effects));
            }
            else {
                player.sendPublicChatMessage(new PublicChatMessage(Utils.capitalize(message), effects));
            }
            ...
            if (Settings.DEBUG) {
                Logger.log(this, "Chat type: " + chatType + ":" + " " + message);
            }
        }
            ...
            if (Settings.DEBUG) {
                Logger.log(this, "Missing packet " + packetId + ", expected size: " + length + ", actual size: " +
